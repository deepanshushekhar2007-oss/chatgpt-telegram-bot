...
// TTL: 1 hour — avoids serving a stale version after WhatsApp updates its
// server-side requirements on long-running instances.
...
async function getCachedBaileysVersion(): Promise<number[]> {
  if (!_cachedVersion || Date.now() - _cachedVersionAt > VERSION_CACHE_TTL) {
...
async function createSocket(
  ...
  if (!state.creds.registered && pairingMode === "code") {
    ...
    // Strategy: wait for "connecting" state then hold 5s for the WA noise-protocol
    // handshake to complete before requesting the pairing code.
    // Requesting code too early (< 3-4s after "connecting") causes WhatsApp to issue a
    // "fake" code that appears valid but cannot be entered on the phone because the
    // server-side noise handshake has not finished binding the session key.
    // Fallback: request after 15s regardless, for slow/cold-start servers (e.g. Render).
    const fallbackTimer = setTimeout(async () => {
      if (!codeRequestScheduled) {
        codeRequestScheduled = true;
        console.log(`[WA][${userId}] gen=${myGenId} Fallback: requesting pairing code after 15s`);
        await requestCode();
      }
    }, 15000);
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      // Wait 5s after "connecting" so the noise handshake with WA servers fully completes.
      if (connection === "connecting") {
        codeRequestScheduled = true;
        clearTimeout(fallbackTimer);
        console.log(`[WA][${userId}] gen=${myGenId} Socket connecting — requesting pairing code in 5s`);
        setTimeout(async () => {
          await requestCode();
        }, 5000);
      }
...
export async function connectWhatsApp(
  ...
  let existing = sessions.get(resolvedId);

  // If a connectLock is active (e.g. a lazy restore in-flight), wait up to 5s
  // for it to release rather than silently dropping the request. Dropping it
  // means the user never gets their pairing code.
  if (existing?.connectLock) {
    console.log(`[WA][${userId}] connectLock active — waiting up to 5s for it to release`);
    for (let i = 0; i < 50 && sessions.get(resolvedId)?.connectLock; i++) {
      await new Promise((r) => setTimeout(r, 100));
    }
    existing = sessions.get(resolvedId);
    // If still locked after waiting, force-kill the stale session and proceed.
    if (existing?.connectLock) {
      console.log(`[WA][${userId}] connectLock still held after 5s — force-killing stale session`);
...
export async function connectWhatsAppQr(
  ...
  // If a connectLock is active (e.g. a lazy restore in-flight), wait up to 5s
  // for it to release rather than silently dropping the QR request. Dropping it
  // meant the user saw "⏳ Generating QR..." forever with no QR ever appearing.
  if (existing?.connectLock) {
    console.log(`[WA][${userId}] QR: connectLock active — waiting up to 5s for it to release`);
    for (let i = 0; i < 50 && sessions.get(resolvedId)?.connectLock; i++) {
      await new Promise((r) => setTimeout(r, 100));
    }
    existing = sessions.get(resolvedId);
    if (existing?.connectLock) {
      console.log(`[WA][${userId}] QR: connectLock still held after 5s — force-killing stale session`);
      existing.connectLock = false;
    }
  }
  ...
  // Clear any existing MongoDB creds so Baileys starts a completely fresh
  // handshake and always emits a new QR code. Without this, if the user had
  // registered creds in MongoDB (e.g. from an earlier pairing that expired),
  // Baileys would silently try to re-authenticate with those stale creds
  // instead of generating a QR — resulting in a 401 disconnect with no QR
  // ever shown. Clearing upfront guarantees a fresh QR every time.
  console.log(`[WA][${userId}] QR: clearing any stale MongoDB creds before fresh QR pairing`);
  await clearMongoSession(resolvedId).catch((e: any) =>
    console.error(`[WA][${userId}] QR: clearMongoSession error (non-fatal): ${e?.message}`)
  );
...
export async function getGroupIdFromLink(
  ...
  // Backoff schedule (ms) — starts gentle, ramps up to 2 min for heavy rate-limit scenarios
  // 15 attempts total to survive 500-1000 link batch operations
  const BACKOFF = [3000, 8000, 15000, 30000, 45000, 60000, 90000, 120000, 120000, 120000, 120000, 120000, 120000, 120000];
  const MAX_RETRIES = BACKOFF.length + 1; // 15 attempts total
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // Permanent errors — link is genuinely bad, no point retrying
      if (
        msg.includes("gone") ||
        msg.includes("not-found") ||
        msg.includes("invalid invite") ||
      ...
      // Every other error (rate limit, forbidden, conflict, network, timeout,
      // 500/502/503/504/429, socket reset, unknown) is treated as transient —
      // retry with the full backoff schedule so large batch operations survive.
      if (attempt < MAX_RETRIES) {
        const delay = BACKOFF[attempt - 1];
        console.log(`[WA][${userId}] getGroupIdFromLink transient error, waiting ${delay}ms before retry ${attempt + 1}/${MAX_RETRIES}...`);
        await new Promise((r) => setTimeout(r, delay));
        // Re-check session is still alive before next attempt
        const freshSession = useSession(userId);
        if (!freshSession?.socket || !freshSession.connected) {
          console.warn(`[WA][${userId}] getGroupIdFromLink: session lost mid-retry, aborting`);
          return null;
        }
        continue;
      }
