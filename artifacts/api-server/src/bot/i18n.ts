...
async function dbGet(key: string): Promise<string | undefined> {
  try {
    ...
    if (doc && typeof doc.dst === "string") {
      // Reject stale entries that contain unresolved sentinels from the old
      // protect() bug (URL regex ate adjacent sentinels → restore() left ⟦⟦n⟧⟧
      // in the translated text → Telegram 400 "can't parse entities").
      // Auto-healing: returning undefined here causes the caller to re-translate
      // and overwrite the stale entry with a correct value.
      if (doc.dst.includes("\u27E6\u27E6")) {
        console.warn(`[i18n] dbGet: evicting stale sentinel-leaking cache entry ${key}`);
        return undefined;
      }
      return doc.dst;
    }
...
function protect(text: string): { protectedText: string; tokens: string[] } {
  ...
  // 1. HTML tags — protect first so their sentinels are in place before URL scan.
  out = out.replace(/<\/?[a-zA-Z][^>]*>/g, (m) => push(m));
  // 2. URLs (incl. https://chat.whatsapp.com/...)
  //    IMPORTANT: stop at the sentinel-open character ⟦ (U+27E6) so that a URL
  //    immediately followed by a sentinel (e.g. "https://t.me/foo</b>" where
  //    </b> was replaced by ⟦⟦1⟧⟧) does NOT eat the sentinel into the URL token.
  //    Using [^\s\u27E6]+ instead of \S+ for this reason.
...
export async function translate(text: string, lang: Language): Promise<string> {
  ...
  if (hit !== undefined) {
    // Evict stale sentinel-leaking entries that were cached before the protect()
    // URL-regex bug fix. They produce ⟦⟦n⟧⟧ in the final text → Telegram 400.
    if (hit.includes("\u27E6\u27E6")) {
      memCache.delete(key);
      // Fall through to L2/L3 to get (or re-generate) a clean translation.
    } else {
      return hit;
    }
  }
  ...
  const p = (async () => {
    try {
      ...
      // Safety check: if any sentinel survived into the output it means
      // protect()+restore() had a mismatch (e.g. the old URL-eats-sentinel
      // bug). Don't cache a broken result — fall back to the original and
      // negatively cache the key so subsequent calls don't keep retrying.
      if (restored.includes("\u27E6\u27E6")) {
        console.error(`[i18n] translate(${lang}): unresolved sentinels after restore — discarding, returning original`);
...
// NOTE: All other button properties (including the custom `style` field added
// by InlineKeyboard.prototype.text for button colours) are preserved via
// object spread so the custom API server can still apply them.
