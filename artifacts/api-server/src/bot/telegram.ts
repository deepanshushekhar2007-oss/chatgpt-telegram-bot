...
async function startQrPairing(ctx: any, userId: number): Promise<void> {
  ...
  // connectWhatsAppQr is fire-and-forget here: it sets up the socket and the
  // callbacks below handle the async QR/connect/disconnect events. Wrapping in
  // try/catch so a synchronous throw (rare) doesn't leave the user stuck.
  connectWhatsAppQr(
    String(userId),
    async (qr, expiresAt) => {
      ...
      try {
        ...
        active.qrMessageId = sentMsgId;
        active.statusMessageId = undefined;
      } catch (err: any) {
        console.error(`[QR][${userId}] Failed to send QR photo: ${err?.message}`);
        // Reset lock so the next QR update from Baileys can still be shown.
        active.qrLocked = false;
        return;
      }

      active.interval = setInterval(async () => {
        const current = qrPairings.get(userId);
        if (!current || current.qrMessageId !== sentMsgId) return;
...
async function createGroupsBackground(userId: string, numericUserId: number, gs: GroupSettings, chatId: number, msgId: number) {
  ...
  // Stop automatically if 3 consecutive group creations fail — send what was created so far
  let consecutiveCreationFailures = 0;
  const MAX_CONSECUTIVE_CREATE_FAILS = 3;
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      if (result) {
        ...
        consecutiveCreationFailures = 0; // reset on success
        results.push({
          name: groupName,
        ...
        const applyGroupConfig = async () => {
          await new Promise((r) => setTimeout(r, speed === "fast" ? 200 : 1500));
        ...
        // Stop after 3 consecutive failures — send links created so far
        if (consecutiveCreationFailures >= MAX_CONSECUTIVE_CREATE_FAILS) {
          for (let j = i + 1; j < total; j++) {
            results.push({ name: gs.finalNames[j], link: null, error: "Stopped — 3 consecutive failures" });
          }
          i = total - 1;
          break;
        }
    ...
    if (i < total - 1 && !userStates.get(numericUserId)?.groupCreationCancel) {
      if (speed === "slow") {
        // 1 minute between groups — show live countdown with progress bar
        const linesNow = results.map((r) => r.link ? `✅ ${esc(r.name)}` : `❌ ${esc(r.name)}`).join("\n");
        const slowBar = renderProgressBar(progressPct);
        // nextGroupAt = now + 60s — used for precise "time remaining" display
        const nextGroupAt = Date.now() + 60_000;
        for (let tick = 0; tick < 12; tick++) {
          ...
          const secsLeft = Math.max(0, Math.round((nextGroupAt - Date.now()) / 1000));
          try {
            await bot.api.editMessageText(chatId, msgId,
              `⏳ <b>Creating Groups: ${done}/${total}</b>\n${slowBar}\n\n${linesNow}\n\n🐢 <b>Slow mode</b> — next group in <b>${secsLeft}s</b>...`,
              { parse_mode: "HTML", reply_markup: new InlineKeyboard().text("❌ Cancel Creation", "group_cancel_creation") }
            );
          } catch {}
        }
  ...
  const chunks = splitMessage(message, 4000);
  // Try to edit the in-place message (the "Stopping..." or progress screen).
  // If the edit fails (Telegram rate-limit / flood wait after many rapid edits),
  // fall back to sending a new message so the result is NEVER silently dropped.
...
bot.callbackQuery("lv_confirm", async (ctx) => {
  ...
  void (async () => {
    ...
    const successfulLeaveIds: string[] = []; // collect for bulk delete at end

    // ── Leave ALL groups in batches of 15 concurrently ────────────────────────
    // 15 groups at a time — fast and safe. Each group gets up to 3 retries with
    // increasing backoff (1.5s → 3s → 5s). Delete fires all at once in
    // background AFTER all leaves complete — much faster than deleting 1-by-1.
    const LV_CONCURRENT = 15;
    const LV_BATCH_DELAY_MS = 400;    // wait between batches (reduced for speed)
    ...
    for (let batchStart = 0; batchStart < groups.length; batchStart += LV_CONCURRENT) {
      ...
      const lvPct = Math.round((done / groups.length) * 100);
      const lvBar = renderProgressBar(lvPct);
      try {
        await bot.api.editMessageText(chatId, msgId,
          `⏳ <b>Leaving: ${done}/${groups.length}</b>\n${lvBar}\n✅ ${success} left | ❌ ${failed} failed\n\n${lines.slice(-15).join("\n")}`,
          { parse_mode: "HTML", reply_markup: new InlineKeyboard().text("⛔ Cancel", "lv_cancel") }
        );
      } catch {}
    ...
    // ── Delete ALL successfully left groups simultaneously in background ───────
    // Fire all deletes at once (no rate limit on local chat deletion) so it
    // completes instantly instead of stretching across the whole leave run.
    if (successfulLeaveIds.length > 0) {
      void Promise.allSettled(
        successfulLeaveIds.map((id) => deleteGroupChat(String(userId), id).catch(() => {}))
      );
    }
...
async function removeAllGroupMembersBackground(
  ...
  for (let gi = 0; gi < groups.length; gi++) {
    ...
    let rmConsecutiveFails = 0; // rate-limit backoff tracker

    // Process members in batches of 5 concurrently — faster than 1-by-1
    // with retry + exponential backoff on rate-limit errors.
    const RM_BATCH = 5;
    for (let pi = 0; pi < nonAdmins.length; pi += RM_BATCH) {
      if (removeMembersCancelRequests.has(Number(userId))) { cancelledEarly = true; break; }

      const batch = nonAdmins.slice(pi, pi + RM_BATCH);
      const batchResults = await Promise.allSettled(
        batch.map(async (p) => {
          // Retry up to 4 times with escalating backoff on failure (rate limit guard)
          const retryDelays = [600, 1500, 3000, 6000];
          for (let attempt = 0; attempt <= retryDelays.length; attempt++) {
      ...
      // If seeing many consecutive failures, back off harder (rate limit)
      const baseDelay = rmConsecutiveFails >= 3 ? 3000 : 300;

      const processedSoFar = Math.min(pi + RM_BATCH, nonAdmins.length);
      // Update progress every batch
      if (msgId && !cancelDialogActiveFor.has(Number(userId))) {
        try {
          await bot.api.editMessageText(chatId, msgId,
            `⏳ <b>Group ${gi + 1}/${groups.length}: ${esc(group.subject)}</b>\n\n🗑️ Removing: ${processedSoFar}/${nonAdmins.length}\n✅ Removed: ${removed} | ❌ Failed: ${failed}`,
            { parse_mode: "HTML", reply_markup: new InlineKeyboard().text("❌ Cancel", "rm_cancel_request") }
          );
        } catch {}
      }

      if (pi + RM_BATCH < nonAdmins.length) {
...
bot.callbackQuery(/^switch_wa:(.+)$/, async (ctx) => {
  ...
  if (switchConnected) {
    ...
    // ── onError ─────────────────────────────────────────────────────────────
    // Connection failed — remove the failed slot from profile entirely and
    // reconnect the user's main WhatsApp. User must re-add the number fresh.
    try {
      const freshProfile = await loadWaSwitchProfile(userId);
      if (freshProfile) {
        // Remove the failed slot from the list
        freshProfile.slots = freshProfile.slots.filter((s) => s.id !== targetSlotId);
        // Restore alias to previous active slot (or raw uid)
        const fallbackId = currentActiveId && currentActiveId !== targetSlotId ? currentActiveId : uid;
        freshProfile.activeId = fallbackId;
        setSessionAlias(uid, fallbackId);
        await saveWaSwitchProfile(freshProfile);
      }
      // Evict the failed slot socket from memory and clear its alias
      try { await idleDisconnectWhatsApp(targetSlotId); } catch {}
      // Reconnect main/previous WhatsApp in background
      ensureSessionLoaded(currentActiveId ?? uid).catch(() => {});
    } catch {}
    try {
      await bot.api.editMessageText(chatId, msgId,
        `❌ <b>Switch Failed — Number Removed</b>\n\n` +
        `📱 <code>${esc(slot.phone)}</code> connect nahi ho saka.\n\n` +
        "⚠️ Yeh number aapke switch list se remove kar diya gaya hai.\n\n" +
        "✅ Aapka pehla wala WhatsApp reconnect ho raha hai.\n\n" +
        "💡 Agar yeh number dobara use karna hai, toh <b>Add New WhatsApp</b> se nayi se add karo.",
        {
          parse_mode: "HTML",
          reply_markup: new InlineKeyboard()
